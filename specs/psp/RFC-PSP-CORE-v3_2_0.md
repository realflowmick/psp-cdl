...
**Revision note:** This revision resolves signature issues PSP-E001 through PSP-E004 at the originating author's direction. It selects [PSP Signature Profile 2.0](../profiles/PSP-SIGNATURE-2.0.md): exactly three signed fields, complete protected content, strict expiration, and one canonicalization/encoding. Version 3.1.1 remains the archived baseline. Existing signatures require migration and re-signing; see Section 17.7.5.

**Example convention:** Many workflow illustrations abbreviate metadata or use placeholder signatures. They are not complete wire records. Executable signature examples are provided by the profile's shared vectors; complete signed records MUST satisfy Section 17 and profile 2.0.
...
${psp type=system signature="abc123" signature-algorithm="ed25519"
     ...
     timestamp="1699564800" expires="1699651200" version="v1.2.3" signature-version="2.0" content-type="text"}
...
${psp type=node node-type="application" name="customer_onboarding"
  ...
  ${psp type=node id="greeting" node-type="prompt" version="v1.0.5"}
    ${psp type=system signature="def456" signature-algorithm="ed25519"
         kid="realflow-prod-2024-11"
         expires="1699651200" version="v1.0.5" signature-version="2.0" content-type="text"}
...
${psp type=node node-type="application" name="lead_qualification"
  ...
  ${psp type=system signature="sig1" signature-algorithm="ed25519" signature-version="2.0" content-type="text"}
...
```
${psp type=system id="node1" signature="abc123" signature-version="2.0" content-type="text"}
```
...
```
${psp type=system signature="..." signature-algorithm="ed25519" version="v1.0.0" signature-version="2.0" content-type="text"}
...
```
${psp type=context signature="..." signature-algorithm="ed25519" signature-version="2.0" content-type="text"}
Customer: John Doe
...
${psp type=node node-type="application" name="healthcare_intake" 
  ...
  ${psp type=system signature="..." signature-algorithm="ed25519"
       kid="compliance-2024" version="v1.0.0" signature-version="2.0" content-type="text"}
...
${psp type=node node-type="application" name="customer_service"
  ...
  ${psp type=system signature="..." signature-algorithm="ed25519"
       kid="realflow-prod-2024-11" version="v2.0.0" signature-version="2.0" content-type="text"}
...
${psp type=node id="validate_input" node-type="prompt" version="v1.2.3"}
  ${psp type=system signature="..." signature-algorithm="ed25519"
       kid="realflow-prod-2024-11" version="v1.2.3" signature-version="2.0" content-type="text"}
...
${psp type=node id="collect_contact_info" node-type="prompt" version="v1.0.0"}
  ${psp type=system signature="..." signature-algorithm="ed25519"
       kid="realflow-prod-2024-11" version="v1.0.0" signature-version="2.0" content-type="text"}
...
${psp type=node id="onboarding" node-type="composite" version="v2.1.0"}
  ${psp type=system signature="..." signature-algorithm="ed25519"
       kid="realflow-prod-2024-11" version="v2.1.0" signature-version="2.0" content-type="text"}
...
${psp type=node id="route" node-type="prompt" version="v1.1.0"}
  ${psp type=system signature="..." signature-algorithm="ed25519"
       kid="realflow-prod-2024-11" version="v1.1.0" signature-version="2.0" content-type="text"}
...
${psp type=node node-type="application" name="customer_service"
  ...
  ${psp type=node id="check_another" node-type="reset" version="v1.0.0"}
    ${psp type=system signature="..." ... signature-version="2.0" content-type="text"}
...
${psp type=node node-type="application" name="customer_returns"
  ...
  ${psp type=node id="authenticate" node-type="prompt" version="v1.0.0"
    ...
    ${psp type=system signature="..." signature-algorithm="ed25519"
         kid="realflow-prod-2024-11" version="v1.0.0" trust-level="2" priority="80" signature-version="2.0" content-type="text"}
  ...
  ${psp type=node id="refund_routing" node-type="prompt" version="v1.0.0"
       ...
       transition-endpoints="mcp://erp/*,mcp://crm/*"}
    ${psp type=system signature="..." signature-algorithm="ed25519"
         kid="realflow-prod-2024-11" version="v1.0.0" trust-level="2" priority="90" signature-version="2.0" content-type="text"}
...
${psp type=output signature="..." signature-algorithm="ed25519"
     kid="realflow-prod-2024-11" timestamp="1700741400" expires="1700827800" signature-version="2.0" content-type="json"}
{
  "workflow_status": "completed",
  ...
}
...
This revision selects [PSP Signature Profile 2.0](../profiles/PSP-SIGNATURE-2.0.md) as the normative signing contract. Profile 2.0 requires Ed25519 and permits HMAC-SHA256. The other algorithms discussed below are extension candidates and require separately specified algorithm profiles before use; they are not accepted by the base profile.
...
**Example**:
```
${psp type=system signature="a3f2c1..." signature-algorithm="hmac-sha256" secret-id="internal-service-2024-q4"
     timestamp="1699564800" expires="1699651200" version="v1.2.3" signature-version="2.0" content-type="text"}
Internal service instruction
${/psp}
```

#### 17.1.3 Other Supported Algorithms
...
#### 17.3.1 Canonicalization and Protected Content

Implementations MUST follow [PSP Signature Profile 2.0](../profiles/PSP-SIGNATURE-2.0.md). `canonical_content` represents the complete protected section, including its metadata; it is not merely the body between opening and closing tags.

Construct the JCS object `{"data": normalized_data, "metadata": protected_metadata}`. Protected metadata includes signature-format version, algorithm, key/secret identifier, section type, content type, expiration, effective trust level, priority, and all additional section attributes. Exclude only the signature value and the separately signed timestamp and content version. Expand omitted trust level to 2, priority to 50, and additional attributes to an empty object before canonicalization.

Text bodies normalize CRLF and CR to LF and trim only ASCII tab, LF and space at both ends; internal whitespace and all other characters are preserved. JSON data is canonicalized under RFC 8785. Neither representation performs Unicode normalization. Section 17.8 and the profile specify exact field types, duplicate handling, delimiter mapping and UTF-8 serialization.

#### 17.3.2 Signature Input

Ed25519 and HMAC-SHA256 use exactly the same three-field input:
...
Ed25519 signs these bytes directly. HMAC-SHA256 computes a MAC over these bytes. No separate application prehash is applied. Trust level, priority, expiration and key identity are already bound inside `canonical_content` and MUST NOT be appended as extra fields.

The timestamp is canonical unsigned decimal Unix seconds. The version is a valid SemVer 2.0.0 content version with a single optional lowercase `v` prefix removed; prerelease and build suffixes are preserved. Timestamp and version cannot contain a pipe; pipes within content remain ordinary signed data. See profile 2.0 for ranges and precise normalization.

#### 17.3.3 Signature Encoding

Profile 2.0 uses canonical unpadded base64url exclusively. Reject whitespace, padding, standard-Base64-only characters, nonzero unused pad bits, and incorrect decoded lengths. Ed25519 produces 64 bytes; HMAC-SHA256 produces 32 bytes. Hexadecimal and other Base64 forms are legacy encodings and MUST NOT be guessed or tried as fallbacks.

### 17.4 Signature Verification

#### 17.4.1 Verification Process

1. Strictly parse required fields, signature-version 2.0, field types, ranges, and canonical signature encoding; reject duplicate names, unsupported fields and conflicting aliases.
2. Resolve the key or secret through the verifier's trusted registry in authenticated application/tenant context. Reject unknown or revoked keys and disallowed algorithms.
3. Reconstruct the complete protected content and exactly `canonical_content + "|" + timestamp + "|" + canonical_version` as defined in Section 17.3.
4. Verify the Ed25519 signature or compare the HMAC-SHA256 output in constant time.
5. Apply Section 17.4.2 and independently enforce the key's allowed trust levels, section types and scope before processing content.
6. Verify nested envelopes independently. A valid outer signature does not waive an inner signature's expiration, key authorization or scope checks.

A cryptographically valid signature authenticates the declaration; the trusted key registry determines whether its signer has authority to make it. Untrusted content, claimed model state or an attacker-supplied key registry cannot grant authority.

#### 17.4.2 Timestamp Validation

Every signed section MUST contain `timestamp` and protected `expires` as integer Unix seconds within the interoperable safe-integer range, with `expires > timestamp`. Missing, malformed or inverted values are invalid.

After signature verification, use:

```python
# Metadata and skew configuration have already been validated.
if now >= expires:
    return SIGNATURE_EXPIRED
if now < timestamp - configured_clock_skew:
    return SIGNATURE_NOT_YET_VALID
return TIME_VALID  # Other authority/scope checks still apply.
```

The validity interval is `timestamp - configured_clock_skew <= now < expires`. Keep subsecond precision in `now`; do not round down to extend validity. Default skew is zero; trusted verifier configuration MAY allow up to 300 seconds for a slightly future timestamp only. Expiration is never extended. At the exact expiration instant, the signature is expired.

`refresh-grace` is a pre-expiration refresh lead time, not permission to execute expired content. If a fresh authorized signature is unavailable at expiration, protected execution MUST pause or fail closed. Audit-only verification may report a mathematically valid but expired signature; it MUST NOT authorize execution.

### 17.5 Key Management

#### 17.5.1 For Asymmetric Algorithms (Ed25519, RSA, ECDSA)
...
- SHOULD maintain historical keys for an audit-retention period (recommended: 90 days); this never extends execution validity
...
- After audit-retention period, mark old keys as archived (verify only) or revoked (reject)

#### 17.5.2 For Symmetric Algorithms (HMAC)
...
All signed delimiter sections require `signature`, `signature-algorithm`, `signature-version="2.0"`, `type`, `content-type`, `timestamp`, `expires`, and `version`. Ed25519 requires `kid`; HMAC-SHA256 requires `secret-id`. The unused alternative key identifier MUST be absent. `trust-level` and `priority` are optional with defaults 2 and 50. All additional section attributes are protected inside canonical content.

See [profile 2.0](../profiles/PSP-SIGNATURE-2.0.md) for the corresponding JSON names, normalization and restrictions. Missing signature-format versions are legacy, not implicit profile-2.0 messages.

#### 17.6.1 Key Identifier (kid)
...
- SHOULD retain historical public keys for an audit-retention period (e.g., 90 days) to inspect historical signatures; retention MUST NOT authorize expired content
...
4. After audit-retention period (e.g., 90 days), archive or revoke old `kid`
...
${psp type=system
     ...
     expires="1699651200"
     version="v1.2.3" signature-version="2.0" content-type="text"}
You are a helpful assistant.
${/psp}
```

Example with HMAC (required secret-id identifies the configured secret):
```
${psp type=system
...
${psp type=system signature="..." signature-algorithm="ed25519"
     ...
     timestamp="1733300000" expires="1733386400" version="v1.0.0" signature-version="2.0" content-type="text"}
...
```
${psp type=system signature="..." trust-level="1" priority="50" ... signature-version="2.0" content-type="text"}
You are a financial advisor for Acme Corp.
${/psp}

${psp type=system signature="..." trust-level="1" priority="90" ... signature-version="2.0" content-type="text"}
All advice must comply with SEC regulations.
Never recommend unregistered securities.
${/psp}

${psp type=system signature="..." trust-level="1" priority="70" ... signature-version="2.0" content-type="text"}
...
#### 17.7.3 Signature Coverage

Trust level, priority, section type, expiration and key identity are included within the canonical protected content. They are not separate concatenated signature fields. Changing an effective value changes the signed bytes and invalidates the signature. Expanding an omitted default to its explicit equivalent preserves the signature because its effective meaning is unchanged.

Authority is derived from the trusted key registry for the verified key identifier. A signer MUST be authorized for the claimed trust level, section type and application/tenant scope. Even a valid signature from another authority cannot elevate a section's privileges. Context-bound operations must check signed scope attributes against independently authenticated context.

#### 17.7.4 Inference Engine Enforcement
...
#### 17.7.5 Backward Compatibility

Profile 2.0 explicitly authenticates complete section metadata inside the first of three fields. Older signatures over body-only content or five-field inputs are not wire compatible. Reissue documents using an authorized signing service, an explicit `signature-version="2.0"` and the profile's canonical content. Missing, unknown or legacy profile identifiers MUST NOT silently fall back to another formula or encoding.

Legacy readers may be isolated for migration or audit, but cannot claim profile-2.0 validation or grant its authority to unverified legacy metadata. The content version and signature-format version are independent. Optional trust/priority defaults remain 2/50 and are included in canonical content.

Reference vectors and migration rules are in [PSP Signature Profile 2.0](../profiles/PSP-SIGNATURE-2.0.md).
...
#### 17.8.1 Standard JSON Payload Structure
...
2. **PSP Tag Format**: Wrap the JSON payload in signed PSP delimiter tags (e.g., `${psp type=context signature="..." ... signature-version="2.0" content-type="json"}{"key": "value"}${/psp}`)

Both formats are valid after the profile-2.0 mapping; use explicit `content-type` for delimiter bodies. The JSON envelope format is particularly useful for pure API integrations where PSP delimiter tags may not be practical.
...
| `data` | string/object/array | Yes | Text or JSON data selected by protected `contentType` |

#### 17.8.2 Signature Object Fields

| Attribute | Type | Required | Description |
| --- | --- | --- | --- |
| `value` | string | Yes | Canonical unpadded base64url signature |
| `algorithm` | string | Yes | `ed25519` or `hmac-sha256` |
| `kid` | string | Ed25519 only | Trusted public-key registry identifier |
| `secretId` | string | HMAC only | Trusted secret registry identifier |
| `timestamp` | integer | Yes | Unix seconds; second signed field |
| `expires` | integer | Yes | Protected exclusive expiration, greater than timestamp |
| `version` | string | Yes | SemVer content version; third signed field after normalization |
| `signatureVersion` | string | Yes | Exactly `"2.0"` |
| `sectionType` | string | Yes | Registered lowercase PSP section type |
| `contentType` | string | Yes | `"text"` or `"json"` |
| `trustLevel` | integer | No | 0–5, defaults to 2 inside canonical content |
| `priority` | number | No | 0–100, defaults to 50 inside canonical content |
| `attributes` | object | No | Additional section attributes as string values; defaults to `{}` |

Unknown members, null alternatives, duplicate names, and reserved names repeated in `attributes` are rejected. Copy all members except `value`, `timestamp` and `version` to protected metadata, expanding defaults. Every operative section attribute must be signed, whether in a defined field, protected attributes, or data.

#### 17.8.3 Extended Attribute Names (x-prefix)
...
Accept exactly one matched pair: `signature`/`data` or `x-signature`/`x-data`. Normalize either pair to the same internal representation. Reject mixed pairs, simultaneous aliases, duplicate property names and additional envelope-root members. Application content and controls belong inside signed data or protected attributes.

#### 17.8.4 Canonicalization for JSON Payloads

Use RFC 8785 JCS on the complete protected-content object described in Section 17.3 and [profile 2.0](../profiles/PSP-SIGNATURE-2.0.md). Sorted-key serialization alone is not an alternative. Preserve Unicode string data exactly; do not normalize to NFC. Reject duplicate names before parsing discards them, invalid Unicode, and unsupported numeric values.
...
Text data uses the profile's explicit line-ending and ASCII boundary-whitespace rules. JSON data preserves string contents. Trust level 2, priority 50 and empty additional attributes are expanded into protected metadata when omitted. Both transports sign the same mapped section, not just the raw JSON data object.

#### 17.8.5 MCP Tool Call Integration
...
#### 17.8.6 API Request/Response Integration
...
#### 17.8.8 Verification Process for JSON Payloads

Apply the same strict parsing, three-field input construction, algorithm/key-policy verification, expiration and scope checks as Section 17.4. Reject at `now >= expires`. A JSON transport does not relax required attributes or introduce a separate signing formula. Verify nested envelopes recursively and independently.

#### 17.8.9 Schema Definition

The normative structural schema is [psp-signature-envelope-2.0.schema.json](../../schemas/psp-signature-envelope-2.0.schema.json). It covers both matched wire aliases, required metadata, key-identifier selection, text/JSON data types and numeric ranges. JSON Schema alone cannot establish cryptographic authenticity or detect duplicate names discarded by a parser.

Implementations MUST additionally enforce profile 2.0's canonical byte/encoding rules, `expires > timestamp`, duplicate detection, trusted-key authority, expiration and scope. Passing structural validation is not signature verification.

#### 17.8.10 Interoperability with PSP Delimiter Format
...
| `type` | `signature.sectionType` |
| `content-type` | `signature.contentType` |
| (additional attributes) | `signature.attributes` |
...
Profile 2.0's protected-content boundary applies to both encryption orders. Encrypt-then-sign MUST authenticate ciphertext and all interpretation parameters before decryption; carry verified provenance through decryption rather than claim the original signature signs newly produced plaintext. Sign-then-encrypt MUST encrypt and recover a complete signed envelope and verify that recovered envelope before use. Unverified outer routing hints do not grant trust or authority.
...
${psp type=system encrypted="true" 
     ...
     version="v1.0.0" trust-level="2" priority="50" signature-version="2.0" content-type="text"}
...
1. Construct and sign the complete plaintext section under profile 2.0, including its protected metadata.
2. Encrypt the entire signed envelope, including its signature value and metadata.
3. The outer encryption container is not itself a verified PSP signature envelope. Decrypt it, then verify the recovered inner envelope; outer routing hints remain untrusted.

```
${psp type=system encrypted="true" encryption-algorithm="aes-256-gcm"
     encryption-key-id="org-key-2024" nonce="..." tag="..."
     version="v1.0.0"}
[encrypted complete profile-2.0 envelope: signature metadata + signature value + plaintext data]
${/psp}
```
...
${psp type=system encrypted="true" encryption-algorithm="aes-256-gcm"
     ...
     timestamp="1765068284" expires="1765154684" version="v2.1.0" signature-version="2.0" content-type="text"}
<encrypted ciphertext payload>
${/psp}
...
For encrypt-then-sign, the encryption parameters and ciphertext are inside the signed `data` object. They MUST NOT be unprotected envelope-root siblings.
...
Verify the complete profile-2.0 envelope before using any encryption parameter. This JSON data shape and the delimiter ciphertext/attribute shape are distinct signed representations; converting between those two shapes requires re-signing. A direct delimiter-to-JSON-envelope mapping under Section 17.8.10 preserves the same text data and protected attributes and therefore preserves bytes.

For sign-then-encrypt, the decrypted bytes must recover a complete signed envelope, which is then independently verified. The outer encryption container's metadata is not an authority claim.

#### 17.9.8 Encryption and Trust Levels

Encryption does not elevate trust. For encrypt-then-sign, verify the outer signature and its authorized trust/scope before decryption and retain that provenance through the transformation. Verify any additional inner signed envelopes independently. For sign-then-encrypt, outer trust hints remain untrusted; enforce only the trust level authorized by the recovered, verified inner envelope.
...
```
${psp type=system signature="..." version="1.0.0" signature-version="2.0" content-type="text"}
...
${psp type=system encrypted="true" decrypt="node"
     ...
     version="v2.1.0" signature-version="2.0" content-type="text"}
...
3. **Maintain public key archive** for N days (e.g., 90 days) for audit-only verification of historical signatures
...
The following pseudocode delegates parsing, canonicalization and policy to profile-2.0 helpers; it is not an implementation of those helpers.

```python
def verify_psp_signature(section, execution_context, now, clock_skew=0):
    parsed = parse_profile_2_strict(section)
    key = trusted_registry.resolve(parsed.key_id, execution_context)
    enforce_key_status_and_algorithm(key, parsed.algorithm)
    canonical = canonicalize_protected_section(parsed)  # All metadata included.
    version = canonical_semver(parsed.version)
    message = utf8(canonical + "|" + decimal(parsed.timestamp) + "|" + version)
    verify_signature_or_mac(key, parsed.algorithm, message, parsed.signature)
    require_valid_interval(parsed.timestamp, parsed.expires, now, clock_skew)
    require_authorized_claims_and_scope(key, parsed, execution_context)
    return {"status": "valid"}

def refresh_signed_node(node_id, version, execution_context):
    node = trusted_nodes.fetch_authorized(node_id, version, execution_context)
    issued_at = integer_unix_seconds()
    # The signing authority chooses and authenticates expiration.
    section = make_profile_2_section(node, timestamp=issued_at,
                                     expires=issued_at + 72 * 60 * 60)
    return sign_complete_protected_section(section)
```

### 21.7 Refresh Lead Time and Expired Content

A configured `refresh-grace` begins refresh before expiration. It MUST NOT authorize execution after expiration. At `now >= expires` the section is expired; if refresh is unavailable, pause or fail closed. Do not implement warning-only post-expiration execution.

Historical signatures may be checked for audit, but MUST be reported as expired and cannot authorize workflow actions. Retaining a historical public key does not extend any signature's validity.
...
${psp type=system signature="..." signature-algorithm="ed25519"
     ...
     refresh-interval="10"
     refresh-grace="300"
     version="1.0.0" signature-version="2.0" content-type="text"}
System instructions here
${/psp}
```
...
${psp type=system signature="[FRESH_SIGNATURE]" signature-algorithm="ed25519"
     ...
     refresh-policy="interval|checkpoint"
     refresh-interval="10"
     version="1.0.1" signature-version="2.0" content-type="text"}
...
result = mcp.call("realflow.security.scan", {
  raw_text: `
    ...
    ${psp type=system signature="..." signature-algorithm="ed25519" kid="key1" version="v1.0.0" signature-version="2.0" content-type="text"}
    System instructions here
    ${/psp}
...
Implementations MUST verify signatures before processing signed sections using the complete protected content and three-field formula in profile 2.0.
...
Signed sections MUST include protected `expires` and signed `timestamp`, with `expires > timestamp`. Expiration is exclusive and has no execution grace period.
...
Complete protected content MUST be canonicalized using JCS (RFC 8785), without Unicode normalization. Sorted-key serialization alone is not an alternative.
...
- Maintain a public key archive for audit retention; expired content remains ineligible for execution
...
```
${psp type=system version="1.0.0" signature="..." signature-algorithm="ed25519" signature-version="2.0" content-type="text"}
...
- `signature-algorithm`
- `signature-version`
- `content-type` (text or json for signed sections)
- `timestamp`
- `expires`
- `version`
- `name`
...
- `signatureVersion` - Required signature profile, 2.0 for this revision
- `sectionType` - Protected section type
- `contentType` - Protected body interpretation (text or json)
- `attributes` - Protected additional section attributes

### 30.3 PSP Signature Algorithm Registry

A registry for signature algorithms SHOULD be maintained to ensure interoperability:

Registered algorithm names (profile 2.0 permits only Ed25519 and HMAC-SHA256; other entries require separate profiles):
...
Initial encryption algorithms defined by this specification:
...
## Revision History: 3.2.0

- Resolved PSP-E001: one three-field signing input; trust and authority metadata are protected inside canonical content.
- Resolved PSP-E002: complete metadata coverage, including expiration, profile, section type, key identity and additional attributes; independent signer authorization remains mandatory.
- Resolved PSP-E003: mandatory bounded times, exclusive expiration, limited issuance clock skew, pre-expiration refresh only, and audit-only handling of expired signatures.
- Resolved PSP-E004: RFC 8785 JCS, preserved Unicode, canonical unpadded base64url, strict duplicates/aliases, and versioned migration.
- Added a normative signature profile, structural envelope schema and cross-language test vectors. Existing body-only or five-field signatures require authorized re-signing.
- Preserved the previous 3.1.1 document and its import hashes as historical provenance.
