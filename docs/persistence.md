...
| Fresh operation/policy/evidence bindings | Stop a previously allowed operation executing after state changes | Resolver/dispatch integration is next; existing read-only results are not permits |
...
The new library is available to host applications now. Public session/checkpoint HTTP/MCP tools remain unadvertised until they have authentication, transition authorization, operation snapshot invalidation and transport tests. Next, add that integration and controlled cancellation/listing/cleanup; then add the PostgreSQL adapter and dispatch outbox. Package publication, production readiness and complete PSP conformance remain pending.
