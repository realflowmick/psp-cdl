...
This is a library API for an application host. The opt-in [MCP stdio adapter](mcp-stdio.md) now connects it to local caller and downstream processes. The [HTTP adapter](mcp-http.md), [approved revision refresh](mcp-revision.md) and [buffered model/tool loop](llm-loop.md) provide additional opt-in slices. OAuth client flows and complete topology enforcement remain pending. It requires no SaaS account, provider credential or external service.
