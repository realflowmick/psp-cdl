...
The first [MCP dispatch gate](mcp-dispatch.md) depends on core/CDL and the explicit workflow store API. An optional `OwnerCoordinator` shared by every writer and gate prevents competing owner transitions through the full local read-only dispatch/release interval. Endpoint identity and capabilities come from host-authenticated immutable registrations. Per-call policy resources, recipient capabilities and revision checks are supplied outside model input. The gate buffers a bounded object, emits level-5 local provenance, and rejects mutating endpoints. It is not yet a network MCP proxy, model loop or complete topology implementation; distributed dispatch and durable effect recovery remain separate work.
