...
The [stateful library foundation](persistence.md) supplies tenant/owner-scoped sessions, immutable nodes, atomic versioned updates and single-use checkpoints. The opt-in [workflow API](workflow-api.md) connects it to authenticated HTTP/MCP methods, exact-transition authorization, private token handoff and session-bound operation issuance/invalidation. The separate lifecycle and security-tools drafts below add listing/cancellation/retention and scan/decrypt/process. Complete workflow conformance, normative API adoption, independent security review and effectiveness evaluation remain future work.
