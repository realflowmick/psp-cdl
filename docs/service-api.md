...
See the complete [OpenAPI draft](../schemas/api/security-0.1.openapi.json) and [MCP schemas](../schemas/mcp/security-tools-0.1.json). A JSON-RPC protocol error is separate from a tool-execution error; tool errors contain stable codes without backend exception text. Full workflow methods, including state mutation and decryption, remain unsupported and unadvertised.
...
The [stateful library foundation](persistence.md) is now available: tenant/owner-scoped sessions, immutable nodes, atomic versioned updates, single-use checkpoint resumption and replay tests using a portable SQLite backend. It is a trusted-host API, not a bearer-authenticated service. Public stateful tools remain unadvertised. Next, connect authenticated transition authorization and session-bound operation snapshot issuance/invalidation to that store. Scan/decrypt/process require separate contracts and key-management decisions. Complete proxy mediation, all eleven required PSP tools, Streamable HTTP MCP, independent security review and effectiveness evaluation remain future work.
