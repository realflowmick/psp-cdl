...
Authenticated HTTP/MCP security, opt-in workflow adapters and the first host-embedded MCP dispatch gate now consume these libraries; see the [service guide](service-api.md), [workflow API](workflow-api.md) and [MCP dispatch guide](mcp-dispatch.md). `McpDispatchGate` in both languages checks exact active-node affinity, registered capability unions, input/output schemas and CDL at dispatch and release. `OwnerCoordinator` is an optional persistence helper required when attaching a store to this single-process gate. Full RFC MUST coverage, remaining M3 tools, network proxy mediation, the model loop, inference-engine attention isolation and measured attack resistance remain later work. A profile result is scoped library evidence, not full protocol certification.
