...
`tokenizeDeclaration` / `tokenize_declaration` returns lexical terms in first-occurrence order: it splits strings on ASCII whitespace, trims array entries, lowercases ASCII letters, drops empty entries and deduplicates. It retains the existing bounded token grammar (64 KiB, 1,024 terms, 128 characters per term). It does not infer a declaration kind, validate registry membership or authorize negation. For example, `"PII no-training"` becomes `["pii", "no-training"]`; callers must still use the typed policy APIs and host-owned authority for enforcement. Fifteen [shared lexical vectors](../conformance/vectors/codec/cdl-lexical-0.1.json) cover this API separately from the profile report.

The typed string and array declaration formats normalize to the same sorted tokens. JSON transport preserves all fields without claiming that every annotation is supported. `resolveSchemaPolicy` / `resolve_schema_policy` resolves **one data JSON Pointer** through `properties` and homogeneous `items`; it is not JSON Schema validation or whole-object classification. Hosts must validate their data separately, resolve every contributing location, and reject unsupported schema features.
...
The opt-in [MCP prompt refresher](llm-mcp-refresh.md) supplies the refresh host callback through an explicitly approved peer/catalog and owner/session binding. It does not add refresh to the model's tools; the loop retains all signature, compatibility and release checks.
