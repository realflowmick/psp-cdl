...
Next, reconcile the still-open #37/#39 acceptance checklists against their merged
evidence, and record remaining requirements explicitly. #34 needs normative
inventory/parser dispositions; #38 has merged draft implementations but retains
...
building on the merged fixtures after the relevant #34/#39 acceptance dispositions.
It must retain explicit blocked/unsupported results for unresolved requirements.

## Remaining work

**Review** identifies outstanding normative dispositions. **Reconcile** means the implementation is merged but the issue acceptance checklist remains open. **Coverage** extends merged infrastructure after its normative prerequisites. **Next** identifies the selected conformance target, subject to its dependencies. **Later** work has its own contract and dependency gates. **Deferred** candidates need explicit scope decisions. **Release** tasks prepare and review artifacts without authorizing publication.

| Queue | Milestone | Task | Dependency or scope gate |
| --- | --- | --- | --- |
| Review | M1 | [#34 Normative inventory and parser-contract review](https://github.com/realflowmick/psp-cdl/issues/34) | #54 merged the inventory/proposals; record normative dispositions while preserving RFC baselines |
| Review | M1 | [#35 Missing API reference and editorial errata](https://github.com/realflowmick/psp-cdl/issues/35) | #58 review artifacts merged; comment window and recorded adoption decision remain pending |
...
| Reconcile | M3 | [#39 Isolated governed/adversarial MCP servers](https://github.com/realflowmick/psp-cdl/issues/39) | #56 merged; reconcile acceptance evidence and documented isolation limits |
| Reconcile | M3 | [#37 Session listing, cancellation and retention](https://github.com/realflowmick/psp-cdl/issues/37) | #57 merged; reconcile lifecycle/persistence acceptance evidence |
...
| Next | M6 | [#48 Workflow conformance and topology harness](https://github.com/realflowmick/psp-cdl/issues/48) | #34 disposition and #39 acceptance; fixtures merged, unavailable combinations remain explicit |
...
The merged opt-in [session lifecycle draft](lifecycle.md) adds owner-scoped listing, cancellation and bounded policy-approved payload cleanup in both languages. Checkpoint/operation invalidation, replay tombstones and retained metadata have explicit contracts. The #37 acceptance checklist remains open; this slice does not complete M3 or claim physical erasure.
