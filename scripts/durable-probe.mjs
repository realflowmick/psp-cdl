...
import {DurableLlmLoop,RedirectingLlmLoop,LoopError} from '@psp-cdl/llmproxy';
...
const host={authenticate:()=>({...options.actor,scopes:['sessions:read','sessions:write','models:invoke']}),now:()=>100,
  ...
  resolveRedirect:unexpected,redirectPolicy:unexpected,
...
try {
  const store=new WorkflowStore(backend,{resumeSecret:new Uint8Array(32).fill(42),authorizePersistence:()=>true,coordinator:new OwnerCoordinator(),durableTurns:true,redirectTurns:!!options.redirect});
  const gate=new McpDispatchGate(store,host,'r1',[]);
  const Loop=options.redirect?RedirectingLlmLoop:DurableLlmLoop;
  const loop=new Loop(store,gate,host,{id:'mock',revision:'model-1',complete:true,sources:[],invoke:unexpected},options.redirect?{postCompletion:'redirect',target:'mcp://realflow/applications/support'}:{postCompletion:'lockdown'});
  ...
  catch(e){if(!(e instanceof LoopError))throw e;denied={code:e.code,...(e.response?{response:e.response}:{})};}
