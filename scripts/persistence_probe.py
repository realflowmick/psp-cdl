    ...
    def permit(*_):
        if barrier := options.get("barrier"):
            ...
            while not Path(barrier["go"]).exists():
                ...
                time.sleep(0.02)
        return True
    store = WorkflowStore(backend, resume_secret=SECRET, authorize_persistence=permit, durable_turns=bool(options.get("durableTurns")), prompt_refresh=bool(options.get("promptRefresh")), redirect_turns=bool(options.get("redirectTurns")), scoped_turns=bool(options.get("scopedTurns")))
    try:
