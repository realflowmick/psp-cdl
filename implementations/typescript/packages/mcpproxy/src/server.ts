...
export function createMcpProxyService(gate:McpDispatchGate,sessionId:string,controls:()=>CallOptions):McpToolService {
...
export function createMcpProxy(gate:McpDispatchGate,credential:()=>string,sessionId:string,controls:()=>CallOptions):McpServer {
  return new McpServer(createMcpProxyService(gate,sessionId,controls),credential);
}
