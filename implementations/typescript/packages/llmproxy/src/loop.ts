...
import { WorkflowStore, OwnerCoordinator, StoreError, bounded, integer } from "@psp-cdl/api-server/persistence";
...
export class BufferedLlmLoop {
  ...
  private async verify(p:Principal,binding:Record<string,unknown>,prompt:Envelope):Promise<void> {
    ...
    try {
      ...
      const e=verifyEnvelope(prompt,{...policy,context,allowedAttributes:Object.keys(context),now:this.host.now()});
  ...
  async run(token:unknown,sessionId:string,request:unknown,options:LoopOptions):Promise<Record<string,unknown>> {
    try {
      ...
      this.check(options);
      const initial=await this.coordinator.run(actor,async()=>{
      ...
      const {session,authority,binding,prompt}=initial,expires=Math.min(authority.expires,session.expiresAt as number);
      const fresh=async()=>{
      ...
      for(let step=1;step<=options.maxSteps;step++) {
        const stepBinding={...binding,step,promptDigest:bindingDigest(prompt)};
        const result=await this.coordinator.run(actor,async()=>{
          await fresh();
