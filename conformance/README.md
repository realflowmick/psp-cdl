...
The [requirement review](../specs/reviews/PSP-CDL-REVIEW-0.1.md) audits all 443 uppercase RFC keyword occurrences and retains explicit blocked/unimplemented obligation status; extraction is not conformance. All five seed vectors remain draft expectations. All five now have scoped mappings in the [offline A/B/C workflow matrix](../docs/workflow-matrix.md). CDL-002 executes lexical tokenization without inferring declaration kinds or authority. The version 0.2 matrix accounts for all 460 requirements, including partial evidence mappings for 27 and explicit pending whole-clause dispositions. The harness reports inventory separately and the default full-conformance command still exits 2. Blocked, unsupported, skipped, and error outcomes must never become passes.
