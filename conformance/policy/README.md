...
| `evaluate` | One resolved originating policy's `classes`, `covenants`, `capabilities`, trusted `checks`, `parameters`, and `context`; returns the profile decision and ordered reason codes. Positive checks are `satisfied`, `failed` or `unknown`; absence is unsatisfied. |
| `evaluate-batch` | Separate `resources` evaluated conjunctively. Keep legal-basis alternatives and parameters local to their original resource. The current batch fixture contains one failing resource; future multi-failure cases must follow the batch combination rule below. |
