...
The [buffered LLM loop guide](docs/llm-loop.md) adds the first M5 slice: provider-neutral callbacks, host-bound signed system text, transcript-wide CDL checks, existing gate-backed tools and buffered final authorization. The [paired mock examples](examples/llm/README.md) need no provider account. 69 shared cases compare actual calls, suppressed output and provider-visible transcripts. Streaming, durable completion, automatic refresh and live-provider adapters remain unsupported.
