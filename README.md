...
**Status: proposed standards; experimental reusable libraries.** TypeScript and Python implement PSP markup/object/JSON codecs, signatures and deterministic CDL policy evaluation. Reusable HTTP/MCP adapters expose authenticated security checks and opt-in workflow operations on durable SQLite sessions, nodes and single-use checkpoints. The first MCPProxy library gate adds host-approved read-only tool dispatch and buffered output checks. Hosts supply identity, transition approval and permitted data views. Full proxy transports, the model loop and remaining M3 tools are pending. Library tests establish scoped behavior, not production readiness, full protocol conformance or measured security effectiveness. This repository is independent of the sponsor's commercial SaaS; no RealflowCloud account is required.
...
The [MCP dispatch guide](docs/mcp-dispatch.md) covers the first M4 slice: namespaced local registrations, exact active-node affinity, CDL checks, bounded schemas, output provenance and optional process-local coordination shared with workflow writes. [Paired examples](examples/dispatch/README.md) show one permitted and one denied tool call. Mutating tools, network proxy mediation and durable dispatch recovery remain unsupported.
