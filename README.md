...
The [workflow API](docs/workflow-api.md) adds six opt-in HTTP/MCP session, node and checkpoint methods, host transition authorization and session-bound operation handles. Run the [paired examples](examples/workflow/README.md) for create â†’ save â†’ pause â†’ authenticated resume. 41 shared workflow scenarios and real HTTP/stdio mutations execute in both language directions. This is an embeddable reference layer; managed product operations and customer experience belong to the host.
